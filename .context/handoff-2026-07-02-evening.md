...
5. **Desktop zoom cluster fix**: sub-bar −/+/fit was disabled in EVERY view since it shipped — `setZoomApi(null)` on view change ran AFTER the incoming view registered (child effects before parent effects) and nulled the fresh api. DesktopWorkspace now clears only for the master edit grid; master.tsx reset deleted. Zoom buttons got aria labels ("zoom in/out/fit"). `shot-zoom.mjs` asserts per-view zoom (canvas scale fingerprint ×1.25; Timeline px/yr 9→11).
6. **APK instant-crash fixed (df7a905)**: expo-notifications ^57→~56.0.19 (SDK-56 native mismatch, statically imported at launch via notifications/reminders.ts); faceEngine.native tfjs imports now fully lazy (tfjs-react-native pulled react-native-fs/expo-gl at startup through the router bundle); eas.json dev/preview pin applicationArchivePath to app-universal-*.apk (ABI splits emit 5 APKs, EAS grabbed arbitrary one). **Worklets pin now 0.8.3** (was 0.8.1). USER: rebuild `eas build -p android --profile preview`; if still crashing, `adb logcat -b crash`.
7. **Task #8 DONE — export redesign**: `exportData.ts` light/dark palettes + photo-embedding avatarSVG (data:-URIs only — remote refs don't render in svg-as-img + taint PNG canvas), new `buildNetworkSVG`, radial depth 1–5 + centre param; `buildBookletHTML` (cover/monogram/stats → per-view SVG-data-URI pages, no rasterisation → generation directory w/ photos) replaces buildDirectoryHTML; ExportPanel: Network option, Light/Dark pills, centre picker + depth slider, PDF-booklet tile; NetworkView live app avatars show photos. `shot-export.mjs` verifies. User answers: theme toggle / depth+person picker / full booklet / photos everywhere.
